      ...
      c = {
        ...
        checkAccount: function() {
          ...
          if (this.billingAddress) {
            this.billingAddress.userId = this.customer._id;
            if (this.billingAddress._id != null) {
              $http.put('/api/addresses/' + this.billingAddress._id, this.billingAddress);
            } else {
              $http.post('/api/addresses/', this.billingAddress);
            }
          }
          if (this.shippingAddress) {
            console.log(this.shippingAddress);
            this.shippingAddress.userId = this.customer._id;
            if (this.shippingAddress._id != null) {
              $http.put('/api/addresses/' + this.shippingAddress._id, this.shippingAddress);
            } else {
              $http.post('/api/addresses/', this.shippingAddress);
            }
          }
          return cart.stage += 1;
        },
